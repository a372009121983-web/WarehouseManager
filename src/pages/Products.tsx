...
import { Package, Plus, Edit2, Trash2, Search, Upload, CheckCircle, XCircle, X, Layers } from 'lucide-react';
...
const CARD_COLORS = [
  { from: '#10b981', to: '#047857', glow: 'rgba(16,185,129,0.45)' },
  { from: '#3b82f6', to: '#1d4ed8', glow: 'rgba(59,130,246,0.45)' },
  { from: '#8b5cf6', to: '#6d28d9', glow: 'rgba(139,92,246,0.45)' },
  { from: '#f59e0b', to: '#b45309', glow: 'rgba(245,158,11,0.45)' },
  { from: '#f43f5e', to: '#be123c', glow: 'rgba(244,63,94,0.45)' },
  { from: '#06b6d4', to: '#0e7490', glow: 'rgba(6,182,212,0.45)' },
  { from: '#f97316', to: '#c2410c', glow: 'rgba(249,115,22,0.45)' },
  { from: '#ec4899', to: '#be185d', glow: 'rgba(236,72,153,0.45)' },
];
...
const Products = () => {
  ...
  const emptyForm = {
    ...
    min_sale_price: 0, max_sale_price: 0, quantity: 0,
  };
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filtered.map((product, i) => {
          ...
          const qtyStatusColor = qtyStatus === 'نافد' ? '#fca5a5' : qtyStatus === 'منخفض' ? '#fde68a' : '#bbf7d0';
          const clr = CARD_COLORS[i % CARD_COLORS.length];

          return (
            <div key={product.id}
              className="relative rounded-2xl overflow-hidden hover:scale-[1.03] active:scale-[0.98] transition-all duration-200 animate-fade-up flex flex-col select-none"
              style={{
                background: `linear-gradient(135deg, ${clr.from} 0%, ${clr.to} 100%)`,
                boxShadow: `0 8px 32px ${clr.glow}, 0 2px 8px rgba(0,0,0,0.15)`,
              ...
              <div className="absolute -left-4 -bottom-5 w-20 h-20 rounded-full pointer-events-none"
              ...
              <div className="relative p-4 flex-1">
                ...
                <div className="flex items-start justify-between gap-2 mb-3">
                  <div className="flex items-center gap-2.5 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="font-bold text-sm text-white leading-tight truncate">{product.name}</p>
                      {product.category && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded-md mt-0.5 inline-block font-medium"
                  ...
                  <div className="text-left flex-shrink-0">
                    <p className="text-xl font-black text-white leading-none">{totalQty.toLocaleString('ar-EG')}</p>
                    <p className="text-[10px] mt-0.5 font-semibold" style={{ color: qtyStatusColor }}>
                      {qtyStatus}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 mb-2.5">
                  <div className="rounded-xl p-2.5" style={{ background: 'rgba(255,255,255,0.12)' }}>
                    <p className="text-[9px] mb-0.5 font-medium" style={{ color: 'rgba(255,255,255,0.65)' }}>سعر الشراء</p>
                    <p className="text-xs font-bold text-white truncate">
                      {product.purchase_price ? EGP(product.purchase_price) : '—'}
                    </p>
                  </div>
                  <div className="rounded-xl p-2.5" style={{ background: 'rgba(255,255,255,0.2)' }}>
                    <p className="text-[9px] mb-0.5 font-medium" style={{ color: 'rgba(255,255,255,0.65)' }}>سعر البيع</p>
                    <p className="text-xs font-bold text-white truncate">{EGP(product.price)}</p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(product as any).unit && (
                    <span className="text-[10px] px-2 py-0.5 rounded-lg font-medium"
                      style={{ background: 'rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.88)' }}>
                      {(product as any).unit}
                    </span>
                  )}
                  <span className="text-[10px] px-2 py-0.5 rounded-lg font-medium"
                    style={{ background: 'rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.88)' }}>
                    حد: {product.min_stock || 0}
                  </span>
              ...
              {/* Card Footer */}
              <div className="relative px-4 pb-4 pt-3 flex gap-2"
            ...
            <div className="p-6 grid grid-cols-2 gap-3">
              ...
              <div className="col-span-2 flex flex-col gap-1.5">
                ...
                <p className="text-[10px] text-slate-400">
                  {editItem ? 'سيتم تحديث الكمية في المخزن الرئيسي مباشرة' : 'الكمية الابتدائية في المخزن الرئيسي — لنقلها لاحقاً استخدم التحويلات'}
                </p>
              </div>
            </div>
